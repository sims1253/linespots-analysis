...
setwd("~/Documents/dev/linespots/linespots-analysis/projpred/")
...
cvs1$vind
suggest_size(cvs1)
p1 = varsel_plot(cvs1, stats = c('elpd', 'rmse'), deltas=T)
p2 = mcmc_areas(as.matrix(projpred1), pars = c('(Intercept)', names(cvs1$vind[1:suggest_size(cvs1)]), 'sigma'))
pdf("projpred-1.pdf")
...
cvs2$vind
suggest_size(cvs2)
p1 = varsel_plot(cvs2, stats = c('elpd', 'rmse'), deltas=T)
p2 = mcmc_areas(as.matrix(projpred2), pars = c('(Intercept)', names(cvs2$vind[1:suggest_size(cvs2)]), 'sigma'))
pdf("projpred-2.pdf")
grid.arrange(p1, p2, ncol=1)
dev.off()
