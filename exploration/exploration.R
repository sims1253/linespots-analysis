...
setwd("~/Documents/dev/linespots/linespots-analysis/exploration/")
...
ei25.intervals = mean(d$EInspect25EXAM) + (seq(-4, 4, 2) * sd(d$EInspect25EXAM))
ei25.intervals
pdf("exp-ei25.pdf")
p = ggplot(d, aes(x = EInspect25EXAM)) +
  geom_density(color = "grey22")
foo = ggplot_build(p)$data[[1]]
p = p +
  geom_area(
    data = subset(foo, x >= ei25.intervals[2] & x <= ei25.intervals[4]), aes(x = x, y = y), fill = "grey") +
  geom_vline(aes(xintercept = median(EInspect25EXAM)), linetype = "solid", color = "grey32") +
  ggtitle("Density of EInstpect@25", "with median, 2 sd interval")
p
dev.off()


hdm.intervals = mean(d$hdMaxEXAM) + (seq(-4, 4, 2) * sd(d$hdMaxEXAM))
hdm.intervals
pdf("exp-hdm.pdf")
p = ggplot(d, aes(x = hdMaxEXAM)) +
  geom_density(color = "grey22") + scale_x_log10()
foo = ggplot_build(p)$data[[1]]
p = p +
  geom_area(
    data = subset(foo, x >= hdm.intervals[2] & x <= hdm.intervals[4]), aes(x = x, y = y), fill = "grey") +
  geom_vline(aes(xintercept = median(hdMaxEXAM)), linetype = "solid", color = "grey32") +
  ggtitle("Density of hdMaxEXAM", "with median, 2 sd interval")
p
dev.off()